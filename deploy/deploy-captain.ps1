...
$jobs = Cloud @('scheduler','jobs','list',"--project=$project","--location=$region",'--format=value(name)')
$name = "projects/$project/locations/$region/jobs/captain-planner"
if ($jobs -notcontains $name) {
    ...
    Cloud @('scheduler','jobs','create','http','captain-planner',"--project=$project","--location=$region",'--schedule=0 0 1 1 *','--time-zone=Europe/London',"--uri=$origin/captain/control/tick",'--http-method=POST',"--oidc-service-account-email=captain-planner@$project.iam.gserviceaccount.com","--oidc-token-audience=$origin",'--attempt-deadline=90s','--max-retry-attempts=0','--quiet')
}
Cloud @('scheduler','jobs','pause','captain-planner',"--project=$project","--location=$region",'--quiet')
Cloud @('scheduler','jobs','update','http','captain-planner',"--project=$project","--location=$region",'--schedule=*/1 * * * *','--time-zone=Europe/London','--quiet')
